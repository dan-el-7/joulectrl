...
- **In flight:** `joulectrl layouts` CLI subcommand committed: class_map_from_topology() converts A's fixture, lists the 4 layout candidates (table or --json). 81 B-owned tests green.
- **Resume here:** `git status --short; git pull --rebase origin main` — if fixtures/real/calibration_c2.json exists, run `python -c "from core.sweep_check import check_calibration_files; r=check_calibration_files('fixtures/real/calibration_c2.json','fixtures/real/calibration_c1.json'); print(r.ok, r.problems, r.warnings)"` and post the result; else check AGENTS2 for the team's next open item (all B gate-items delivered).
